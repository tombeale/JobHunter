...
namespace JobHunter.Application
{
    public static class ToolBars
    {
        ...
        public static List<MenuItem> GetCampaignRowItems(Campaign campaign, bool hasNotes = false, bool isDone = false)
        {
            string NoteImage = (hasNotes) ? "Note-Filled" : "Note";

            List<MenuItem> list = new List<MenuItem>();
            list.Add(new MenuItem("", "Edit",    "Edit this action",   "", ""));
            if (!String.IsNullOrEmpty(campaign.RefUrl)) list.Add(new MenuItem("", "WWW", "List/Show Actions",  campaign.RefUrl, ""));
        ...
        public static List<MenuItem> GetCampaignViewActionItems(bool hasNotes = false, bool isDone = false)
        {
            string NoteImage = (hasNotes) ? "Note-Filled" : "Note";
            string DoneImage = (isDone)   ? "Add" : "Close";
